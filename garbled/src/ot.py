...
class ObliviousTransfer:
    ...
    def get_result(self, a_inputs, b_keys):
        ...
        """
        logging.debug("Sending inputs to Bob", extra={"author": "ALICE"})
        self.socket.send(a_inputs)

        for _ in range(len(b_keys)):
            # Receive gate ID where to perform OT
            w = self.socket.receive()
            logging.debug(f"Received gate ID {w}", extra={"author": "ALICE"})
    ...
    def send_result(self, circuit, g_tables, pbits_out, b_inputs):
        ...
        logging.debug("Received Alice's inputs", extra={"author": "BOB"})

        for w, b_input in b_inputs.items():
            logging.debug(f"Sending gate ID {w}", extra={"author": "BOB"})
            # Send gate where to perform OT
            self.socket.send(w)
        ...
        logging.debug("Sending circuit evaluation", extra={"author": "BOB"})
    ...
    def ot_garbler(self, msgs):
        ...
        """
        logging.debug("OT protocol started", extra={"author": "ALICE"})
        G = util.PrimeGroup()
        ...
        logging.debug("OT protocol ended", extra={"author": "ALICE"})
    ...
    def ot_evaluator(self, b):
        ...
        """
        logging.debug("OT protocol started", extra={"author": "BOB"})
        ...
        logging.debug("OT protocol ended", extra={"author": "BOB"})
        return mb
